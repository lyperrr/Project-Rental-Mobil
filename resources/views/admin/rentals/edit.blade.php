...
<div class="container-fluid">
  ...
  <div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 text-gray-800 font-weight-bold">Edit Penyewaan</h1>
      ...
      <form action="{{ route('admin.rentals.update', $rental->id) }}" method="POST">
        ...
        <div class="row">
          <div class="col-md-6 mb-3">
            ...
            <input type="date" name="start_date" id="start_date" class="form-control" value="{{ old('start_date', $rental->start_date->format('Y-m-d')) }}" required>
          </div>
          <div class="col-md-6 mb-3">
            <label for="end_date" class="form-label">Tanggal Selesai Sewa</label>
            <input type="date" name="end_date" id="end_date" class="form-control" value="{{ old('end_date', $rental->end_date->format('Y-m-d')) }}" required>
          </div>
        </div>

        <div class="mb-3">
          <label for="total_price" class="form-label">Total Harga</label>
          <input type="number" name="total_price" id="total_price" class="form-control" value="{{ old('total_price', $rental->total_price) }}" required>
        </div>
        ...
        <div class="mb-3">
          ...
          <select name="status" id="status" class="form-control" required>
            ...
            @foreach (['pending','confirmed','cancelled','completed'] as $status)
              <option value="{{ $status }}" {{ old('status', $rental->status) == $status ? 'selected' : '' }}>
                {{ ucfirst($status) }}
              </option>
            @endforeach
          </select>
